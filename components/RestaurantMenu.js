      ...
      <div className="section lg:mx-28">
        <div className="container mx-auto py-10">
          ...
          <div className="bg-gray-100 rounded-lg p-5">
            <div className="flex flex-wrap lg:flex-cols gap-10 justify-center items-center pt-5">
              <div className="">
                <h2 className="font-semibold text-2xl text-center pb-2">
                  {restaurant?.name}
                </h2>
                <span className="text-center"> {restaurant?.cuisines?.toString()}</span>
                <h4>{restaurant?.costForTwoMessage}</h4>
                <h4>{restaurant?.avgRating} ⭐</h4>
              </div>
              <div className=" pt-8">
                <div>
                  {restaurant?.aggregatedDiscountInfo?.descriptionList?.length >
                    0 &&
            ...
            <p className=" text-center py-5 ml-5">Address:  {restaurant?.labels?.[1]?.message}</p >

            <div className="text-center p-5 flex justify-center">
              <img
                src={IMAGE_CDN + restaurant?.cloudinaryImageId}
                className="rounded-xl shadow-xl h-full w-96"
              />
            </div>
          </div>
          <h1 className="text-center font-semibold text-xl py-10">
            Restaurant Menu
          </h1>
          <div className="lg:flex justify-center  lg:flex-wrap ">
              {restaurantMenu &&
                ...
                restaurantMenu.map((menu) => (
                  ...
                  >
                    <img src={Image_ASSET_CDN + menu?.card?.info?.imageId} className="" />
                    <div>
                      <h4 className="font-semibold">{menu?.card?.info?.name}</h4>
                      <h5 className="font-semibold">Rs. {menu?.card?.info?.price / 100}</h5>
                      <p>{menu?.card?.info?.description}</p>
                      <button className="p-2 m-2 bg-orange-400 text-white rounded-lg" onClick={()=>handleCart(menu?.card?.info)}>Add Item</button>
                    </div>
                  </div>
                ))}
          </div>
